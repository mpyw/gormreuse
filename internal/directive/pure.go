...
// hasGormDBParameter returns true if the signature has any parameter containing *gorm.DB.
...
// hasGormDBReturn returns true if the signature has any return value containing *gorm.DB.
...
// containsGormDB checks if the given type contains *gorm.DB recursively.
func containsGormDB(t types.Type) bool {
	return containsGormDBWithCache(t, make(map[types.Type]*cacheEntry))
}

// cacheEntry represents a cached result for containsGormDB.
// inProgress=true means we're currently checking this type (cycle detection).
// inProgress=false means we have a cached result.
type cacheEntry struct {
	inProgress bool
	result     bool
}

// containsGormDBWithCache is the recursive implementation with cycle detection and result caching.
