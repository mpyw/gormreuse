...
func multiAssign01(db *gorm.DB) {
	db = db.Session(&gorm.Session{}) // Make parameter immutable for testing pure directive
	//gormreuse:pure
...
// =============================================================================
// FUNCTION ARGUMENT CLOSURE PATTERNS
// Tests for closures passed directly as function arguments (not assigned to variable)
// =============================================================================

// Helper function that takes a closure with *gorm.DB argument and calls it
func applyToDB(fn func(q *gorm.DB), db *gorm.DB) {
	fn(db)
}

// Helper function that takes a closure returning *gorm.DB
func callReturningDB(fn func() *gorm.DB) *gorm.DB {
	return fn()
}

// Helper that takes closure with *gorm.DB arg and returns *gorm.DB
func callTransform(fn func(q *gorm.DB) *gorm.DB, db *gorm.DB) *gorm.DB {
	return fn(db)
}

// =============================================================================
// PURE DIRECTIVE ON FUNCTION ARGUMENT CLOSURES
// =============================================================================

// funcArgNoPure: No directive, closure receives mutable q - should report on q.Find
func funcArgNoPure(db *gorm.DB) {
	q := db.Where("base")
	applyToDB(func(x *gorm.DB) {
		_ = x
	}, q)
	q.Find(nil) // want `\*gorm\.DB instance reused after chain method`
}

// funcArgPureBeforeLine: Directive on line before closure argument
// LIMITATION: Marking the closure as pure does NOT help when passed through another function.
// The analysis only checks if the CALLEE (applyToDB) is pure, not the closure argument.
// This is because the analysis doesn't track that applyToDB passes its *gorm.DB arg to the closure.
func funcArgPureBeforeLine(db *gorm.DB) {
	q := db.Where("base")
	applyToDB(
		//gormreuse:pure
		func(x *gorm.DB) {
...
// funcArgPureInline: Directive after opening brace (inline pattern)
// BUG: Directive is recognized (no unused warning) but doesn't take effect
func funcArgPureInline(db *gorm.DB) {
	q := db.Where("base")
	applyToDB(func(x *gorm.DB) { //gormreuse:pure
		_ = x
	}, q)
	q.Find(nil) // want `\*gorm\.DB instance reused after chain method`
}
...
// =============================================================================
// IMMUTABLE-RETURN DIRECTIVE ON FUNCTION ARGUMENT CLOSURES
// =============================================================================

// funcArgNoImmutableReturn: No directive - should report
func funcArgNoImmutableReturn(db *gorm.DB) {
	q := callReturningDB(func() *gorm.DB {
		return db.Session(&gorm.Session{}).Where("setup")
	})
	q.Find(nil)
	q.Count(nil) // want `\*gorm\.DB instance reused after chain method`
...
// funcArgImmutableReturnBeforeLine: Directive on line before closure argument
// LIMITATION: The immutable-return directive on the closure is recognized,
// but the analysis tracks the CALL to callReturningDB, not the closure's return behavior.
// The result q comes from callReturningDB, which is not marked as immutable-return.
func funcArgImmutableReturnBeforeLine(db *gorm.DB) {
	q := callReturningDB(
		//gormreuse:immutable-return
		func() *gorm.DB {
			return db.Session(&gorm.Session{}).Where("setup")
...
// funcArgImmutableReturnInline: Directive after opening brace (inline pattern)
// BUG: Directive is recognized (no unused warning) but doesn't take effect
func funcArgImmutableReturnInline(db *gorm.DB) {
	q := callReturningDB(func() *gorm.DB { //gormreuse:immutable-return
		return db.Session(&gorm.Session{}).Where("setup")
	})
	q.Find(nil)
	q.Count(nil) // want `\*gorm\.DB instance reused after chain method`
...
// =============================================================================
// COMBINED DIRECTIVE ON FUNCTION ARGUMENT CLOSURES
// =============================================================================

// funcArgCombinedNoDirective: No directive - both should report
func funcArgCombinedNoDirective(db *gorm.DB) {
	base := db.Where("x")
	result := callTransform(func(q *gorm.DB) *gorm.DB {
		return q.Session(&gorm.Session{})
	}, base)
	base.Find(nil)    // want `\*gorm\.DB instance reused after chain method`
	result.Find(nil)
...
// funcArgCombinedBeforeLine: Both directives on line before closure
// LIMITATION: Same as above - directives on the closure don't affect how
// callTransform is treated. The analysis tracks callTransform, not the closure.
func funcArgCombinedBeforeLine(db *gorm.DB) {
	base := db.Where("x")
	result := callTransform(
		//gormreuse:pure,immutable-return
...
// funcArgCombinedInline: Combined directive after opening brace
// BUG: Directive is recognized (no unused warning) but doesn't take effect
func funcArgCombinedInline(db *gorm.DB) {
	base := db.Where("x")
	result := callTransform(func(q *gorm.DB) *gorm.DB { //gormreuse:pure,immutable-return
		return q.Session(&gorm.Session{})
	}, base)
	base.Find(nil) // want `\*gorm\.DB instance reused after chain method`
	result.Find(nil)
